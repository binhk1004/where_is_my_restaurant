...
public class MemberLogin extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(result == 1) {
			...
			PrintWriter out = response.getWriter();
			out.println("\r\n" + 
					"<script>alert('아이디 혹은 비밀번호를 다시 확인해주세요.');window.open('about:blank','_self').self.close();opener.document.location.reload();</script>");
			out.flush();	
		}
		else if(result == -1) {
			response.setContentType("text/html; charset=UTF-8");
			PrintWriter out = response.getWriter();
			out.println("<script>alert('등록되지 않은 계정 입니다.');window.open('about:blank','_self').self.close();opener.document.location.reload();</script>");
			out.flush();	
		}
