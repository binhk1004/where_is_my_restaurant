...
public class DeleteReview extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String changeaddress = getaddress.replace('+', ' ');
		String getmembernum = request.getParameter("membernum");
		
		String url = "/ReadReview?address="+URLEncoder.encode(changeaddress, "UTF-8")+"&membernum="+getmembernum;
		
		try {
			int num = Integer.parseInt(request.getParameter("num"));
						
			new Restaurant().delete(num, getmembernum);
